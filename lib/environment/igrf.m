...
% Define a conversion constant between nanoTesla and Tesla
nT2Tesla = 10^-9;

% Specify whether the conversions should be used or not:
%       1 = Yes, 0 = No
optionToConvert_nt2T = 0;

% Run IGRFS if all position inputs are scalars.
if isscalar(latitude) && isscalar(longitude) && isscalar(altitude)
    if nargin < 5
        % --- This is the standard UBNL usage option ---
...
% If enabled, convert nanoTesla to tesla
if optionToConvert_nt2T == 1
    Bx = Bx * nT2Tesla;
    By = By * nT2Tesla;
    Bz = Bz * nT2Tesla;
end

% If there is only a single output, then it is expecting a vector.
% Construct this vector by assembling the different components together.
if nargout <= 1
    Bx = [Bx(:), By(:), Bz(:)];
end
