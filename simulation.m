...
for ii = 1:num_steps
    ...
    % Get target state:
    [desQuat, desAngRate] = EarthTargeter(sat.position, sat.velocity, sat.mu);
...
csvwrite('data/pose_history.csv',data)
csvwrite('data/camera_data.csv',[cam.resolution,cam.sensor,cam.focal])

% Save truth data:
save('data/truth','attitudes','angRates','positions','velocities')
save('data/cam','cam')
save('data/gps','gps_meas')
save('data/gyro','gyro_meas')